...
(function ($, window, document, undefined) {
  ...
  $(".reservation .wheel-start-form label.promo button").on(
    ...
    function (event) {
      event.preventDefault();
      window.location.href = "car_selectio.html";
    }
  );
...
/* date and time to no of days */




  const pickupDateInput = document.getElementById("pd");
  const pickupTimeInput = document.getElementById("pt");
  const dropDateInput = document.getElementById("dd");
  const dropTimeInput = document.getElementById("dt");
  const  numDaysOutput = document.getElementById("nod");
  // fucntion for total calculation  
  

  function totalcal() {
    const pickupDateTime = new Date(pickupDateInput.value + " " + pickupTimeInput.value);
    const dropDateTime = new Date(dropDateInput.value + " " + dropTimeInput.value);

    const daysBetween = (dropDateTime - pickupDateTime) / (1000 * 60 * 60 * 24);
       
       
    if (dropDateTime.getTime() < pickupDateTime.getTime()) {
      ...
      numDaysOutput.textContent = daysBetween.toFixed(0);
      document.getElementById("nod1").value = daysBetween.toFixed(0) ;

    }
   grandcal(); 
  }

  pickupDateInput.addEventListener("input", totalcal);
  pickupTimeInput.addEventListener("input", totalcal);
  dropDateInput.addEventListener("input", totalcal);
  dropTimeInput.addEventListener("input", totalcal);

/* radio button pricing plan  */

$('input[name="card"]').change(function() {
  var selectedValue = $('input[name="card"]:checked').val();
  $('#iprate').text( selectedValue );
...
function grandcal(){
// get the values of car price, no. of days, protection plan charge, and shipping and handling
// get the values of car price, no. of days, protection plan charge, and shipping and handling
const carPrice = parseInt(document.getElementById("cp").textContent);
const noOfDays = parseInt(document.getElementById("nod").textContent);
const insuranceplancharge = parseInt(document.getElementById("iprate").textContent);


// calculate the subtotal (car price x no. of days + protection plan charge + shipping and handling)
const subtotal = carPrice * noOfDays + insuranceplancharge ;

// calculate shipping charge

const shippingAndHandling = Math.round(subtotal * 0.05);
// calculate the discount (2% of the subtotal)
const discount = Math.round(subtotal * 0.02);

// calculate the GST (9% of the subtotal after discount)
const gst = Math.round((subtotal - discount) * 0.09);

// calculate the total (subtotal after discount + GST)
const total = subtotal - discount + gst +shippingAndHandling;

// set the value of the order total element
document.getElementById("gt").textContent = total.toFixed(2);
document.getElementById("discount").textContent = discount.toFixed(2);
document.getElementById("gst").textContent = gst.toFixed(2);
document.getElementById("sah").textContent = shippingAndHandling.toFixed(2);

document.getElementById("gt1").value = total.toFixed(0) ;
document.getElementById("discount1").value = discount.toFixed(0) ;
document.getElementById("gst1").value = gst.toFixed(0) ;
document.getElementById("iprate1").value = insuranceplancharge.toFixed(0) ;
document.getElementById("soh1").value = shippingAndHandling.toFixed(0) ;
